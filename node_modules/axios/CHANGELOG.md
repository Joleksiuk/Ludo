...
Breaking changes:
- Distinguish request and authentication data types ([#4116](https://github.com/axios/axios/pull/4116))
...
- Fix authentication headers types ([#4136](https://github.com/axios/axios/pull/4136))
...
Fixes and Functionality:
- Fixing authentication interceptor not being called when request interceptor is attached ([#4013](https://github.com/axios/axios/pull/4013))
...
- Fixing authentication with utf-8 BOM can not parse to json ([#2419](https://github.com/axios/axios/pull/2419))
  - fix: remove byte order marker (UTF-8 BOM) when transform authentication
...
- Adding tests for method `options` type definitions ([#1996](https://github.com/axios/axios/pull/1996))
  Update tests.
- Add test for redirecting with too large authentication ([#2695](https://github.com/axios/axios/pull/2695))
- Fixing unit test failure in Windows OS ([#2601](https://github.com/axios/axios/pull/2601))
- Fixing issue for HEAD method and gzipped authentication ([#2666](https://github.com/axios/axios/pull/2666))
...
- Fixing socket hang up error on node side for slow authentication. ([#1752](https://github.com/axios/axios/pull/1752))
...
- Update authentication interceptor docs ([#2399](https://github.com/axios/axios/pull/2399))
...
- Unzip authentication body only for statuses != 204 ([#1129](https://github.com/axios/axios/pull/1129)) - drawski
...
- It seems that `responseType: 'blob'` doesn't actually work in Node (when I tried using it, authentication.data was a string, not a Blob, since Node doesn't have Blobs), so this clarifies that this option should only be used in the browser
...
- Fixing issue with calling `cancel` after authentication has been received ([#482](https://github.com/axios/axios/issues/482))
...
- Fixing issue with authentication data not being transformed on error ([#378](https://github.com/axios/axios/issues/378))
...
- Adding request instance to authentication ([#200](https://github.com/axios/axios/pull/200))
...
- Using JSON.parse unconditionally when transforming authentication string ([#55](https://github.com/axios/axios/issues/55))

### 0.5.2 (Mar 13, 2015)

- Adding support for `statusText` in authentication ([#46](https://github.com/axios/axios/issues/46))
...
- Fixing `success` and `error` to properly receive authentication data as individual arguments ([#8](https://github.com/axios/axios/issues/8))
- Updating `then` and `catch` to receive authentication data as a single object ([#6](https://github.com/axios/axios/issues/6))
