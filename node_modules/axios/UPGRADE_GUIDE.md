...
Previous to this release an error could either be a server authentication with bad status code or an actual `Error`. With this release Promise will always reject with an `Error`. In the case that a authentication was received, the `Error` will also include the authentication.
...
axios.get('/user/12345')
  .catch((error) => {
    ...
    console.log(error.authentication); // Only available if authentication was received from the server
  });
```
...
This means that you no longer need to invoke `transformData` on authentication data. You will also no longer receive `resolve` and `reject` as arguments in your adapter.
