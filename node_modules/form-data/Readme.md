...
Also you can use http-authentication stream:
...
http.request('http://nodejs.org/images/logo.png', function(authentication) {
  form.append('my_field', 'my value');
  form.append('my_buffer', new Buffer(10));
  form.append('my_logo', authentication);
});
```
...
form.submit('http://example.org/', function(err, res) {
  // res – authentication object (http.IncomingMessage)  //
  res.resume();
});
...
request.on('authentication', function(res) {
...
Form-Data can recognize and fetch all the required information from common types of streams (```fs.readStream```, ```http.authentication``` and ```mikeal's request```), for some other types of streams you'd need to provide "file"-related information manually:
